...
def _generate_qualifications(ctx: GenerationContext, sector: str) -> List[Dict[str, Any]]:
    faker = ctx.rng.faker
    tasks = ctx.dictionaries.get("tasks", {}).get("tasks", [])
    ...
    for idx, entry in enumerate(slices):
        ...
        qualifications.append(
            {
                "title": entry.get("name", f"Capability {idx+1}"),
                "description": entry.get("deliverables", ["Deliverable focus"])[0],
...
def _generate_schedule(seed_schedule: Dict[str, Any], ctx: GenerationContext) -> Dict[str, Any]:
    rng = ctx.rng
    schedule = common.safe_copy_dict(seed_schedule) if seed_schedule else {}
    start_dt = _random_date_within(rng, rng.randint(2024, 2025))
    end_dt = start_dt + timedelta(days=rng.randint(300, 540))
    ...
    for mistake in mistakes:
        if mistake == "page_limit_violation":
            ...
            if len(parts) >= 2 and parts[-1]:
                ...
                variant = pm_name + " Jr."
            availability = proposal.get("staffing_plan", {}).get("availability", [])
            if availability:
                availability[0] = availability[0].replace(pm_name, variant)
            proposal.setdefault("letter", {}).setdefault("body", [])
            ...
            if orig_date:
                try:
                    new_date = str((datetime.fromisoformat(orig_date) - timedelta(days=rng.randint(5, 25))).date())
                except ValueError:
                    new_date = orig_date
                signature_block["date"] = new_date
                meta.setdefault("consistency", {})["date_mismatch"] = {"expected": orig_date, "actual": new_date}
            ...
            if compliance_matrix:
                entry = rng.choice(compliance_matrix)
                other_sections = [e["expected_section"] for e in compliance_matrix if e is not entry]
                if other_sections:
                    entry["cited_section"] = rng.choice(other_sections)
            ...
            if banned_phrases:
                ...
                proposal.setdefault("work_approach", {}).setdefault("success_factors", []).append(
                    f"This provides an {phrase}."
                )
...
def _summarize_labels(proposal: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    labels = {
        ...
        "date_consistency_flag": "date_mismatch" not in meta.get("consistency", {}),
